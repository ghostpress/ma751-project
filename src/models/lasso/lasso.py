...
class LassoRegression():
    ...
    def __init__(self, X, y, reg, B_0, B):
        """
        Parameters
        ------
        ...
        self.errors = {} # field to track MSE over training iterations
        pass


    @classmethod
    def from_params(cls, X, y, reg, B_0, B):
        return cls(X, y, reg, B_0, B)


    @classmethod
    def new_model(cls, X, y, reg=0.00001):
        # Initialize model parameter values close to zero (can't be = 0 or in GD updates there would be a /0)
        M, N = X.shape
        B = np.zeros(N) + 0.00001
        B_0 = 0.00001
        return cls(X, y, reg, B_0, B)
    ...
    def cross_validation(self, nfolds, param_vals, lr=0.01, iterations=10000):
        ...
        for p in param_vals:
            ...
            for fold in folds.keys():
                ...
                curr_model = LassoRegression.new_model(train_X, train_y, p)
    ...
    def gradient_descent_update(self, lr=0.01, status=False):
        ...
        for j in range(self.N):
            gradB[j] = (-2 * np.dot(self.X[:,j], residual) + self.reg * np.sum(self.B / np.abs(self.B))) / self.M
        
        gradB_0 = (-2 * np.sum(residual)) / self.M
